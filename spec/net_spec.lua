...
describe("net encode/decode round-trips", function()
    ...
    it("MSG_HELLO: preserves the advertised protocol version", function()
        local raw = net.encodeHello(7)
        local msg = net.decode(raw)
        assert.are.equal(net.MSG_HELLO, msg.type)
        assert.are.equal(7, msg.version)
    end)

    it("MSG_HELLO_OK: decodes back to a MSG_HELLO_OK table", function()
        local raw = net.encodeHelloOk()
        local msg = net.decode(raw)
        assert.are.equal(net.MSG_HELLO_OK, msg.type)
    end)

    it("MSG_VERSION_MISMATCH: preserves expected and actual versions", function()
        local raw = net.encodeVersionMismatch(3, 1)
        local msg = net.decode(raw)
        assert.are.equal(net.MSG_VERSION_MISMATCH, msg.type)
        assert.are.equal(3, msg.expectedVersion)
        assert.are.equal(1, msg.actualVersion)
    end)
